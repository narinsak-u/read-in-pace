...
describe('ConfigService', () => {
  ...
  it('exposes typed config from minimal env', () => {
    const config = new ConfigService(validEnv);
  ...
  it('throws when DATABASE_URL is missing', () => {
    expect(() => new ConfigService({ STRIPE_SECRET_KEY: 'sk_x' })).toThrow();
  });

  it('throws when STRIPE_SECRET_KEY is missing', () => {
    expect(() => new ConfigService({ DATABASE_URL: 'postgres://x' })).toThrow();
  });
});
